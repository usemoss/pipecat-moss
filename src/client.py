    ...
    class _MissingDependency:
        def __init__(self, *_: Any, **__: Any) -> None:
            raise RuntimeError(
                ...
                "Install it via `pip install inferedge-moss`."
            )
...
class MossClient:
    ...
    def __init__(
        ...
        if not self._project_id:
            raise ValueError(
                "project_id must be provided or set through the "
                "MOSS_PROJECT_ID environment variable"
            )

        if not self._project_key:
            raise ValueError(
                "project_key must be provided or set through the "
    ...
    async def load_index(self, index_name: str) -> str:
        ...
        if not isinstance(result, str):
            logger.debug(
                "unexpected load_index return type",
    ...
    async def query(
        ...
        logger.debug("querying moss index", index=index_name, top_k=top_k)
        result = await self._client.query(index_name, query, top_k)
        if not isinstance(result, SearchResult) and not hasattr(result, "docs"):
            logger.debug(
                "unexpected query return type",
                type=type(result).__name__,
                index=index_name,
            )
        return result
